...
- **71 / 624** test images were misclassified — see `Diagram/1.jpeg` for examples.
- The confusion matrix (`Diagram/3.jpeg`) shows the model over-predicts PNEUMONIA on NORMAL cases (65 false positives) while rarely missing true PNEUMONIA cases (only 6 false negatives).

## Observations

- Training accuracy climbs smoothly and steadily, while validation accuracy is noisy and volatile across epochs (see `Diagram/4.jpeg` and `Diagram/5.jpeg`), indicating some overfitting and/or a small/imbalanced validation set.
- The model favors recall on PNEUMONIA over precision on NORMAL — it tends to over-flag NORMAL X-rays as PNEUMONIA, as seen in the misclassified examples (`Diagram/1.jpeg`).
